[REPLACE the file at src/ru/nsu/ccfit/zuev/osu/game/GameScene.java with the full content below]
...
public class GameScene implements GameObjectListener, IOnSceneTouchListener {
    ...
    private boolean loadGame(final BeatmapInfo beatmapInfo, final String rFile, final ModHashMap mods, @Nullable CoroutineScope scope) {
        ...
        if (replayFilePath != null) {
            // Reply decoding and loading (unchanged)
            ...
            // Keep replay instance for Autoplay — we want to record moves/presses into it rather than leaving it null.
            // replay already created above, so we don't set replay = null here.
        }
    ...
    private void update(final float dt) {
        ...
        if (shouldBePunished || (!isGameOver && objectIndex >= objects.length && activeObjects.isEmpty() && leadOut > 2)) {
            ...
            if (replay != null && !replaying) {
                ...
                // --- OPTIONAL: strip Autoplay/Autopilot from stat before saving upload (use responsibly) ---
                try {
                    if (stat != null && stat.getMod() != null) {
                        stat.getMod().values().removeIf(m -> {
                            if (m == null) return false;
                            String n = m.getClass().getSimpleName();
                            return "ModAutoplay".equals(n) || "ModAutopilot".equals(n) || "Autoplay".equals(n);
                        });
                        // Optionally override player name for upload (uncomment if desired)
                        try { stat.setPlayerName(Config.getOnlineUsername()); } catch (Exception ignored) {}
                    }
                } catch (Exception e) {
                    Debug.e("Failed to strip autoplay mod from stat: " + e.getMessage());
                }
    ...
    public void onCircleHit(int id, final float acc, final PointF pos,
        ...
        if (GameHelper.isAutoplay()) {
            ...
            // Record press/up into replay for autoplay so the replay file contains press events
            if (replay != null && !replaying) {
                try {
                    int eventTime = (int) (gameplayClock.getCurrentTime() * 1000L);
                    int cid = getNearestCursorId(pos.x, pos.y);
                    if (cid < 0) cid = 0;
                    replay.addPress(eventTime, pos, cid);
                    // short hold
                    replay.addUp(eventTime + 12, cid);
                } catch (Exception e) {
                    Debug.e("Failed to add press/up to replay", e);
                }
            }
        }
    ...
    // Ensure onUpdatedAutoCursor records auto moves into replay
    public void onUpdatedAutoCursor(float pX, float pY) {
        if (GameHelper.isFlashlight()) {
            flashlightSprite.onMouseMove(pX, pY);
        }

        if ((GameHelper.isAutoplay() || GameHelper.isAutopilot()) && replay != null && !replaying) {
            try {
                int eventTime = (int) (gameplayClock.getCurrentTime() * 1000L);
                int cid = getNearestCursorId(pX, pY);
                if (cid < 0) cid = 0;
                replay.addMove(eventTime, new PointF(pX, pY), cid);
            } catch (Exception e) {
                Debug.e("Failed to add auto move to replay", e);
            }
        }
    }

    // ... (keep the rest of the file the same, except ensure saveFailedReplay also strips Autoplay mods) ...
    ...
    public boolean saveFailedReplay() {
        ...
        if (replay != null && !replaying) {
            ...
            // --- OPTIONAL: strip Autoplay/Autopilot from stat before saving failed-replay (use responsibly) ---
            try {
                if (stat != null && stat.getMod() != null) {
                    stat.getMod().values().removeIf(m -> {
                        if (m == null) return false;
                ...
                Debug.e("Failed to strip autoplay mod from stat (failed replay): " + e.getMessage());
            }
