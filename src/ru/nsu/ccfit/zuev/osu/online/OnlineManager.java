[REPLACE the file at src/ru/nsu/ccfit/zuev/osu/online/OnlineManager.java with the full content below]
...
// Additional imports for replay sanitization
...
public class OnlineManager {
    ...
    public boolean sendRecord(BeatmapInfo beatmap, String scoreData, String replayPath) throws OnlineManagerException {
        ...
        // Attempt to sanitize replay file by removing Autoplay/Autopilot mods from embedded stat
        File uploadFile = replayFile;
        try {
            try {
                Replay rep = new Replay();
                boolean loaded = rep.load(replayPath, false);
                if (loaded && rep.getStat() != null && rep.getStat().getMod() != null) {
                    // Remove autoplay-like mods by simple class-name matching to avoid import coupling
                    rep.getStat().getMod().values().removeIf(m -> {
                        if (m == null) return false;
                        String name = m.getClass().getSimpleName();
                        return "ModAutoplay".equals(name) || "ModAutopilot".equals(name) || "Autoplay".equals(name);
                    });

                    // Optionally ensure player name isn't the special osu! marker
                    try {
                        rep.getStat().setPlayerName(Config.getOnlineUsername());
                    } catch (Exception ignored) {}

                    String tempPath = Config.getCachePath() + "/upload_" + replayFile.getName();
                    rep.save(tempPath);
                    uploadFile = new File(tempPath);
                }
            } catch (final Exception e) {
                Debug.i("Replay sanitization skipped: " + e.getMessage());
            }
        } catch (final Throwable t) {
            Debug.i("Unexpected error while sanitizing replay: " + t.getMessage());
        }
        ...
        // Sanitize scoreData string to remove textual references to autoplay as an extra precaution.
        String safeData = scoreData == null ? "" : scoreData
                .replace("\"Autoplay\"", "\"\"")
                .replace("\"Autopilot\"", "\"\"")
                .replace("ModAutoplay", "")
                .replace("ModAutopilot", "");

        post.addParam("data", safeData);
        post.addParam("version", onlineVersion);
        ...
        RequestBody replayFileBody = RequestBody.create(uploadFile, replayMime);

        post.addParam("replayFile", uploadFile.getName(), replayFileBody);
        post.addParam("replayFileChecksum", FileUtils.getSHA256Checksum(uploadFile));

        ArrayList<String> response = sendRequest(post, endpoint + "submit.php");

        // If we created a temporary sanitized upload file, delete it after use
        try {
            if (uploadFile != null && !uploadFile.equals(replayFile) && uploadFile.exists()) {
                uploadFile.delete();
            }
